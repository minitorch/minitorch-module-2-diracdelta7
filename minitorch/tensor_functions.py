...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t1,) = ctx.saved_values
        local_grad = t1.f.add_zip(t1._ensure_tensor(1), t1.f.neg_map(t1))
        local_grad = t1.f.mul_zip(local_grad, t1)
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        (t1,) = ctx.saved_values
        order_list = [int(t1[i]) for i in range(t1.shape[0])]
        inv_order_list = [0] * len(order_list)
        for i, j in enumerate(order_list):
            inv_order_list[j] = i
        return (grad_output.permute(*inv_order_list), 0.0)
